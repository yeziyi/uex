...
//		Map<String, Integer> map = getRunningAppMemory(context);
        StorageInformation storageInformation = new StorageInformation(context, userApps, systemApps, disableApps, handler);
        ...
        for (PackageInfo info : packageInfos) {
            ...
            long totalSize = 0;
            if (storageInformation.sizeMap.containsKey(packageName)) {
                totalSize = storageInformation.sizeMap.get(packageName);
            }
            bean.mTotalSize = totalSize;
            if (bean.mTotalSize <= 0) {
                bean.mTotalSize = 0;
                bean.mFileSize = context.getString(R.string.calculating);
            } else {
